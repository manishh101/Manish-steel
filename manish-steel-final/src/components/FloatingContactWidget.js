...
const WhatsAppIcon = () => (
  <svg className="w-7 h-7 md:w-5 md:h-5" viewBox="0 0 24 24" fill="currentColor">
...
const FloatingContactWidget = () => {
  ...
  const contactOptions = [
    ...
    {
      name: 'Call Us',
      icon: <PhoneIcon className="w-7 h-7 md:w-5 md:h-5" />,
    ...
    {
      name: 'Location',
      icon: <MapPinIcon className="w-7 h-7 md:w-5 md:h-5" />,
    ...
    {
      name: 'Catalog',
      icon: <BuildingOffice2Icon className="w-7 h-7 md:w-5 md:h-5" />,
  ...
  return (
    <div className="floating-contact-widget fixed md:bottom-6 bottom-20 right-6 md:z-50 z-[60]">
      {/* Contact Options */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
          ...
          >
            {contactOptions.map((option, index) => (
              <motion.button
                ...
                className={`${option.color} text-white p-5 rounded-full shadow-lg transform transition-all duration-300 hover:scale-110 flex items-center justify-center group relative backdrop-blur-sm border border-white/20 ${option.name === 'Catalog' ? 'hidden md:flex' : ''}`}
                title={option.label}
                style={{
      ...
      <motion.button
        ...
        className={`w-20 h-20 md:w-18 md:h-18 rounded-full shadow-xl flex items-center justify-center text-white transition-all duration-500 transform hover:scale-110 relative overflow-hidden bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 hover:from-blue-500 hover:via-blue-600 hover:to-blue-700`}
      ...
      >
        ...
        >
          {isOpen ? (
            <XMarkIcon className="w-9 h-9 md:w-8 md:h-8" />
          ) : (
            <div className="relative">
              <ChatBubbleLeftRightIcon className="w-9 h-9 md:w-8 md:h-8" />
